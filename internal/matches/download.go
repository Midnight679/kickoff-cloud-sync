...
// validateReplayURL rejects anything that isn't a plausible HTTPS
// cloud-storage URL. replayURL comes from Epic's match history API —
...
func validateReplayURL(raw string) error {
	...
	if u.Scheme != "https" {
		return fmt.Errorf("expected an https URL, got %q", u.Scheme)
	}
