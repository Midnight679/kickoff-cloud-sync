...
- **Prevention** — `internal/httpclient`'s shared `http.Transport` sets `IdleConnTimeout` to 30s, well under Go's own 90s default, so idle connections are proactively retired client-side before a real-world reverse proxy or CDN's shorter idle timeout has a chance to close them first and cause the race. `matches.DownloadReplay`'s dedicated client (it needs its own `CheckRedirect` for the SSRF re-validation below) still points `Transport` at this same shared instance via `httpclient.Transport()`, rather than falling back to `http.DefaultTransport`'s longer, mismatched idle timeout.
